...
> **Реальный кейс из тестового прогона:** оркестратор несколько раз
> ложно диагностировал «пустой результат», потому что `python3 -c
> "for x in ...: print(x)"` на PowerShell молча не выводит ничего.
> После перехода на `_verify.py`-файлы таких ложных диагнозов больше
> не было.
